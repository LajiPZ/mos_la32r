...
int syscall_read_block(u_int blkno, void *va, u_int nsecs) {
	return msyscall(SYS_read_block, blkno, va, nsecs);
}

int syscall_write_block(u_int blkno, const void *va, u_int nsecs) {
	return msyscall(SYS_write_block, blkno, va, nsecs);
}
